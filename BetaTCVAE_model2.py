...
loss = ELBO + beta*Total_correlation
...
with tf.Session() as sess:
    ...
    for epoch in range(6000):
        ...
        _, tot_loss, loss_likelihood, loss_divergence, toc = \
            sess.run((train_op, loss, marginal_likelihood, KL_divergence, Total_correlation),
    ...
    saver.save(sess, 'TCu1/vaet1')
    print("Model saved")

with tf.Session() as sess:
    new_saver = tf.train.import_meta_graph('TCu1/vaet1.meta')
    new_saver.restore(sess, tf.train.latest_checkpoint('TCu1'))
...
with open('resultTCu1.pckl','wb') as f:
